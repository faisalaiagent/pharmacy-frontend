...
export default function ShopPage() {
  ...
  useEffect(() => {
    setPage(1);
  }, [search, category, minPrice, maxPrice, inStock, rxOnly, ordering]);
        ...
        <div className="space-y-1.5">
          <button
            ...
            className={`w-full text-left text-sm px-3 py-1.5 rounded-lg transition-colors ${
              !category ? "bg-sky-50 text-sky-700 font-medium" : "text-gray-600 hover:bg-gray-50"
            }`}
          >
            All Categories
          </button>
          {(categories || []).map((cat) => (
            <button
              ...
              className={`w-full text-left text-sm px-3 py-1.5 rounded-lg transition-colors ${
                category === cat.slug ? "bg-sky-50 text-sky-700 font-medium" : "text-gray-600 hover:bg-gray-50"
              }`}
            >
              {cat.name}
            </button>
          ...
          {[
            ...
            <label key={opt.label} className="flex items-center gap-2 cursor-pointer">
              <input
                type="radio"
                name="stock"
                checked={inStock === opt.value}
                onChange={() => setInStock(opt.value)}
          ...
          {[
            ...
            <label key={opt.label} className="flex items-center gap-2 cursor-pointer">
              <input
                type="radio"
                name="rx"
                checked={rxOnly === opt.value}
                onChange={() => setRxOnly(opt.value)}
                className="accent-sky-500"
              />
              <span className="text-sm text-gray-600">{opt.label}</span>
            </label>
            ...
            <Sheet open={filtersOpen} onOpenChange={setFiltersOpen}>
              <SheetTrigger asChild>
                <Button variant="outline" className="sm:hidden flex items-center gap-2">
                  ...
                  Filters
                  {activeFiltersCount > 0 && (
                    <Badge className="bg-sky-500 text-white text-xs w-5 h-5 p-0 flex items-center justify-center">
                      {activeFiltersCount}
                    </Badge>
                  )}
                </Button>
          ...
          {activeFiltersCount > 0 && (
            <div className="flex flex-wrap gap-2 mt-3">
              ...
              {rxOnly === "false" && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  OTC Only
                  <X className="w-3 h-3 cursor-pointer" onClick={() => setRxOnly("")} />
                </Badge>
              )}
              {rxOnly === "true" && (
                <Badge variant="secondary" className="flex items-center gap-1">
          ...
          <aside className="hidden sm:block w-56 shrink-0">
            <div className="bg-white rounded-xl border p-4 sticky top-24">
              <div className="flex items-center justify-between mb-4">
                ...
                {activeFiltersCount > 0 && (
                  <Badge className="bg-sky-500 text-white">{activeFiltersCount}</Badge>
                )}
              </div>
              <FilterPanel />
            </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-gray-500">
                {isLoading ? "Loading..." : `${pagination?.count || 0} products found`}
              </p>
                      ...
                      {Array.from({ length: Math.min(5, pagination.total_pages) }, (_, i) => {
                        ...
                        return (
                          <Button
                            ...
                            size="sm"
                            className={`w-8 h-8 p-0 ${page === pageNum ? "bg-sky-500 hover:bg-sky-600" : ""}`}
